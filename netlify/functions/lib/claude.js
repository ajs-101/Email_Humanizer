...
export async function callClaude({ system, messages, maxTokens = 1500, temperature = 0.8 }) {
...
export function stripWrappers(text) {
  let t = (text || '').trim()
  t = t.replace(/^```[a-z]*\n?/i, '').replace(/\n?```$/, '').trim()
  t = t.replace(/^(here('s| is) (the |your |a )?(rewritten|humanized|reply|response|draft)( email| version)?[:.]?\s*\n+)/i, '').trim()
  return t
}
