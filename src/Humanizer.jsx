...
function ScoreChip({ text }) {
  ...
  return (
    <span
      className={`score ${l.tone}`}
      title={`${s.words} words · ${s.sentences} sentences · burstiness ${s.burstiness}`}
    >
...
export default function Humanizer({ onLogout }) {
  ...
  const outScore = useMemo(
    () => scoreEmail(result?.body || ""),
    [result?.body],
  );
  ...
  const run = async () => {
    ...
    try {
      const url = isReply
        ...
        : "/.netlify/functions/humanize";
      const payload = isReply
        ? { email: input, instruction }
        : { email: input, intensity };
      ...
      setResult(
        isReply
          ? { subject: data.subject, body: data.body, read: data.read }
          : { body: data.humanized },
      );
    } catch (e) {
    ...
    <>
      ...
      <main className="main">
        ...
        <section className="glass panel">
          <div className="panel-head">
            <div className="panel-title">
              {isReply ? "Their email" : "Your email"}
            </div>
            <ScoreChip text={input} />
          </div>
          <textarea
            className="editor"
            placeholder={
              isReply
                ? "Paste the email or thread you received..."
                : "Paste the AI-written email here..."
            }
            value={input}
            ...
            <div className="instruct">
              ...
              <div className="flags">
                {QUICK_INSTRUCTIONS.map((q) => (
                  <button
                    key={q}
                    className={`flag chip ${instruction === q ? "on" : ""}`}
          ...
          {inScore.flags.length > 0 && (
            <div className="flags">
              {inScore.flags.slice(0, 10).map((f, i) => (
                <span key={i} className={`flag ${f.type}`}>
                  {f.label}
                </span>
              ))}
              {inScore.flags.length > 10 && (
                <span className="flag">+{inScore.flags.length - 10} more</span>
              )}
            </div>
          )}
          ...
          <div className="panel-foot">
            {isReply ? (
              <span className="meta">
                Replies as Trustpoint Xposure. Company details are built in.
              </span>
            ) : (
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              ...
              {!input && (
                <button
                  ...
                  onClick={() =>
                    setInput(isReply ? SAMPLE_INBOUND : SAMPLE_HUMANIZE)
                  }
                >
                  Try a sample
                </button>
        ...
        <section className="glass panel">
          <div className="panel-head">
            <div className="panel-title">
              {isReply ? "Your reply" : "Humanized"}
            </div>
            <ScoreChip text={result?.body || ""} />
          </div>

          {error ? (
            <div className="output empty" style={{ color: "#fca5a5" }}>
              {error}
            </div>
          ) : result ? (
            ...
            <div className="output empty">
              {busy
                ? isReply
                  ? "Reading their message and drafting a reply..."
          ...
          {outScore.flags.length > 0 && (
            <div className="flags" style={{ marginTop: 12 }}>
              {outScore.flags.slice(0, 8).map((f, i) => (
                <span key={i} className={`flag ${f.type}`}>
                  {f.label}
                </span>
              ))}
